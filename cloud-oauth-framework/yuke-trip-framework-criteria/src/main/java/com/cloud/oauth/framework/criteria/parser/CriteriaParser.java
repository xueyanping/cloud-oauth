...
import com.cloud.oauth.framework.criteria.annotation.Order;
import com.cloud.oauth.framework.criteria.annotation.OrderBy;
import com.cloud.oauth.framework.criteria.factory.CriteriaParserFactory;
import com.cloud.oauth.framework.criteria.observer.Observer;
...
public class CriteriaParser {
    ...
    public <T> QueryWrapper<T> parse(Criteria<T> criteria) {
        return this.parse(criteria, new QueryWrapper<>(), null);
    }

    public <T> QueryWrapper parse(Criteria<T> criteria, QueryWrapper<T> wrapper) {
     ...
     * @return
     */
    public <T> QueryWrapper parse(Criteria<T> criteria, QueryWrapper<T> wrapper, String group) {
     ...
     * @param group
     */
    private void parseOrderBy(Criteria<?> criteria, QueryWrapper entityWrapper, String group) {
        ...
        Arrays.stream(orderbies).filter(orderBy -> StringUtils.equals(StringUtils.trimToNull(orderBy.group()), group) && orderBy.enable()).forEach(orderBy -> {
            entityWrapper.orderBy(true, orderBy.isAsc(), StringUtils.join(orderBy.columns(), ","));
        });
    }
